...
- **Chunk content dedup with embedding reuse** — SHA-256 hash per chunk, reuses existing pgvector embeddings. Backed by `chunks.content_hash` column (migration 011); the column was missing in the original 002 schema, so the SELECT in `ingestion.ts` was silently failing and every duplicate chunk was re-embedding (F4 — cost leak). Migration 011 + INSERT plumbing in `db/vectorStore.ts` close the gap; ingestion now stamps `chunk.contentHash` on every chunk before storage.
- **Configurable charsPerToken ratio** — 3.5 for medical docs, 3.8 for forms, 4.0 default
- **Query route PHI redaction in logger calls** — `routes/query.ts` `logger.info('Query reformulated', ...)` and `logger.info('… query received', ...)` now wrap question/reformulated text with `redactPhi` so raw user input stops flowing to stdout / container log aggregators (F3 / INV-02). Audit + trace persistence already redacted; this closed the last unredacted log path.
- **Gold-standard RAG eval — scheduled CI workflow** — `.github/workflows/eval-rag.yml` runs `npm run eval:rag` weekly (Mondays 06:00 UTC) and on demand via `workflow_dispatch` with adjustable recall/MRR thresholds. Uploads junit.xml + results.json artifacts. Surfaces retrieval regressions before they reach users; not a deploy gate yet (separate signal channel).
...
- **Vision-based image description** — Bedrock Converse API + Haiku 4.5 for PDF images. Now returns `{text, warnings}` instead of bare string (F9): partial-failure paths (PDF too large to chunk, individual chunk failures, total-failure catch) populate `warnings` which bubble to `Document.extractionWarnings`. Same shape from `textExtractor.extractPdf` on pdf-parse failure / OCR failure / OCR confidence < 70%. Frontend renders an amber `⚠ N` badge in DocumentManagerTable so users see when a "ready" document is missing image/page content.
...
- **Default collection startup seeder** — `services/defaultCollection.ts` exports `ensureDefaultCollection()`, called once at server startup after `initializeAuth()` and again lazily from the upload route when the caller falls back to the `'default'` literal. Backs the `documents.collection_id` FK that fires on cold-start uploads to a fresh RDS database where no collection has ever been created. Idempotent, picks the lowest-id admin as `created_by` for deterministic attribution.
...
- **SSO MFA gate** — `middleware/sso.ts` `trySsoIntrospection` refuses to mint a RAG JWT when CA's `/sso-verify` returns `mfaVerified !== true`. Closes a bypass where, with `ENABLE_NATIVE_MFA=false`, RAG would silently accept CA sessions that hadn't completed MFA — RAG's own MFA flow only fires at native login, so the SSO path needed an independent gate (F1).
- **perUserLimiter signature verification** — `server.ts` keyGenerator now calls `verifyToken()` (full signature check) instead of base64-decoding the JWT payload. Closes a targeted-DoS vector where a forged Bearer with `{id: "victim"}` could pool into the victim's rate-limit bucket. Falls through to `resolveRateLimitKey(req)` on verify failure, satisfying the spirit of INV-30 (F5).
- **user.id-vs-username FK bug class** — four route writers passed `req.user!.username` to columns FK'd to `users(id)`, which silently worked in S3-only mode (no FK enforcement) and failed every insert in RDS mode. Fixed in three rounds as RDS uncovered each: `collections.created_by` (PR #146), `documents.uploaded_by` + `ppd_submissions.submitted_by` + `ppd_submissions.reviewed_by` (PR #147), and the matching `listPpdSubmissions` filter so non-admin "list mine" queries match the new id-based rows. Now locked down by INV-38; the safe pattern is the one already used by `logAuditEvent(req.user!.id, req.user!.username, …)` where id goes to FK columns and username goes to display fields.
...
- **Audit log HMAC chain** — HMAC-SHA256 with app secret (not raw SHA-256), mutex-protected writes, S3 write retry with backoff (F-11). S3 fallback chain recovery now scans back N days (default 30, env: `AUDIT_CHAIN_RECOVERY_DAYS`) instead of just today + yesterday; if older audit entries exist beyond the window, fires `audit_chain_fork` operational alert before falling back to GENESIS so multi-day downtime in S3-only mode (no DATABASE_URL) doesn't silently fork the chain (F6).
- **logRagFeedback PHI redaction** — `services/ragTrace.ts` now redacts reviewer-typed `notes` via `redactPhi` before persisting to S3 traces. Closes an asymmetric leak where the companion `saveFeedback()` redacted notes but the trace path didn't, so the same patient text was simultaneously safe in `feedback/` and unsafe in `rag-traces/feedback/` (F2 / INV-02).
...
- **ClamAV PING NUL handling** — `isClamAvailable` and the INSTREAM response parser both stripped NULs before string comparison. ClamAV's `z`-prefixed commands return NUL-terminated responses (`"PONG\0"`), and `String.prototype.trim()` does NOT strip NULs — only whitespace + line terminators. The old `data.toString().trim() === 'PONG'` always failed, caching availability=false for 60s and rejecting every upload. Bug was hidden because no test exercised the real wire protocol; new `__tests__/malwareScan.test.ts` suite stands up a tiny TCP server speaking clamd's z-protocol so the regression is locked down (PR #144).
