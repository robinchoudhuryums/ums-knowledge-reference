...
- **Key services** (`backend/src/services/`):
  ...
  - `defaultCollection.ts` — Idempotent seeder for the `'default'` collection row that the documents UI and upload route fall back to when the user hasn't picked a collection. `ensureDefaultCollection()` is called once at server startup after `initializeAuth()` (the FK on `collections.created_by → users(id)` requires the seeded admin to already exist) and again lazily from the upload route when the caller used the `'default'` literal — handles the edge case where an admin manually deleted the seeded row. Picks the lowest-id admin as creator for deterministic attribution across restarts.
...
INV-38 | All writers to columns FK'd to `users(id)` MUST pass `req.user!.id`, never `req.user!.username`. The bug class historically hit four sites before being caught (`collections.created_by`, `documents.uploaded_by`, `ppd_submissions.submitted_by`, `ppd_submissions.reviewed_by`) — S3-only mode hid them all because there was no FK enforcement; RDS mode fails the constraint on every insert. The route's `logAuditEvent(req.user!.id, req.user!.username, ...)` shape is the safe pattern: id flows to the FK column, username flows to display/log fields | Subsystem: Auth & Security
