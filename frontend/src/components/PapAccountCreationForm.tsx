...
function translateGroup(group: string): string {
  const map: Record<string, string> = {
    ...
    'Sleep Study & Clinical': 'Estudio del Sueño y Clínico',
    'Current Equipment': 'Equipo Actual',
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed['pap1'] === patientName) setResponses(parsed);
      } catch {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-5 sm:px-7">
      <FormHeader
        kicker="PAP / CPAP"
        title={
          lang === 'en' ? 'PAP Account Creation' : 'Creación de Cuenta PAP'
        }
        lang={lang}
                  ...
                  >
                    ...
                    {q.type === 'select' && q.options && (
                      <SelectQuestion
                        value={val}
                        options={q.options}
                        conditionalFormatting={q.conditionalFormatting}
                        onChange={(v) => setResponse(q.id, v)}
                        lang={lang}
                      />
                    )}
